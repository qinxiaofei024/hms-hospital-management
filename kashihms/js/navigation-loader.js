...
class NavigationLoader {
    constructor() {
        ...
        // 为资源请求添加 cache bust，避免浏览器缓存旧的导航/侧边栏
        this.cacheBuster = `?v=${(window.__NAV_LOADER_VERSION__ || Date.now())}`;
        
        if (currentPath.includes('/pages/')) {
            this.navPath = '../components/nav.html' + this.cacheBuster;
            this.sidebarPath = '../js/sidebar.js' + this.cacheBuster;
        } else {
            this.navPath = 'components/nav.html' + this.cacheBuster;
            this.sidebarPath = 'js/sidebar.js' + this.cacheBuster;
        }
    ...
    async init() {
        ...
        try {
            ...
            // 加载导航HTML（禁用缓存）
            const response = await fetch(this.navPath, { cache: 'no-store' });
